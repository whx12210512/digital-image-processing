...
**非几何类别** 也有差距，来自预处理深度不同（App 无 CLAHE/双边滤波/宽域自适应阈值扫描）：

| 类别 | Python | App | 差距 | 根因 |
|------|:------:|:----:|:----:|------|
| barcode_motion_blur | 90.9% | 90.9% | **0%** | 运动模糊对此类影响小 |
| edge_tear | 48.2% | 45.5% | -2.7% | 撕裂本就不易恢复 |
| barcode_scratches | 95.5% | 90.0% | -5.5% | 缺少形态学闭运算连接断条 |
| edge_quiet_zone | 95.7% | 89.0% | -6.7% | 缺少宽域自适应阈值 |
| damage_noise | 70.9% | 61.8% | -9.1% | 缺少 CLAHE + 中值滤波 |
| flaw_motion_blur | 83.9% | 73.9% | -10.0% | 缺少多尺度自适应二值化 |
| ink_corner_destruction | 93.9% | 63.3% | **-30.6%** | 缺少 NCC 定位符修补 |
| ink_data_pollution | 100% | 8.1% | **-91.9%** | 缺少 Otsu + 宽域二值化扫描 |
| **非几何平均** | **84.9%** | **65.3%** | **-19.6%** | |
| | | | | |
| **全部类别平均** | | | **约 -15%** | 几何类 -12%, 非几何类 -20% |
